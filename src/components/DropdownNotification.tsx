...
import { sessionAtom, removeSession, getSession } from '../Store';
...
const DropdownNotification = () => {
  ...
  useEffect(() => {
    const unsub = GetRealTimeNotificationsByID(
      setNotifications,
      session.user_id,
    );
          ...
          >
            <span
              className={`absolute -z-1 inline-flex h-full w-full animate-ping rounded-full opacity-75 ${
                notifications.filter((notif) => notif.is_read === false)
                  .length > 0
                  ? `bg-meta-1`
                  : ``
              }`}
          ...
          <ul className="flex h-auto flex-col overflow-y-auto">
            {notifications &&
              notifications
                ...
                .filter((notif) => notif.user_id === session.user_id)
                .map((notif, idx) => {
                  return (
                    <li key={idx}>
                      <Link
                      ...
                      >
                        ...
                        <p className="text-xs text-alis-blue">
