...
import { UpdateProfilePhoto, GetUserByID, UpdateCoverPhoto } from '../services';
...
const Profile = () => {
  ...
  useEffect(() => {
    const unsub = GetUserByID(session.id, setUser);
  ...
  const handleCoverPhotoUpload = async (e) => {
    ...
    const FS_BUCKET_PATH = '/users/cover-photo';

    if (!file) return null;

    await UpdateCoverPhoto(file, FS_BUCKET_PATH, session);
  };

  const handleProfilePhotoUpload = async (e) => {
    const file = e.target.files[0];
    const FS_BUCKET_PATH = '/users/profile-photo';

    if (!file) return null;

    await UpdateProfilePhoto(file, FS_BUCKET_PATH, session);
  };
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="relative z-20 h-35 md:h-65">
          ...
          {/* <img
            ...
            className="h-full w-full rounded-tl-sm rounded-tr-sm object-cover object-center"
          /> */}
          {/* COMENT-OUT CODE BELOW FOR FUTURE USE */}
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
            <div className="relative drop-shadow-2 h-full">
              {/* PROFILE PICTUR OF USER */}

              {user.profile_photo ? (
                <img
                  src={`${user.profile_photo}`}
                  alt="profile"
                  className="transition duration-200 transform hover:scale-110 rounded-full object-center h-full w-full"
                />
              ) : (
                <img
          ...
          <div className="mt-4">
            ...
            <p className="font-medium">{user.contact_num}</p>
